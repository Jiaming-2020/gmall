...
public class CategoryController {
    private static final String TOP_CACHE_KEY_PREFIX = "pms:topCategories:";
    @Autowired
    ...
    @GetMapping("lv3/{id}")
    public ResponseVo<List<CategoryEntity>> queryCategoriesByLv3Id(@PathVariable("id") Long id) {
        return ResponseVo.ok(categoryService.queryCategoriesByLv3Id(id));
    }

    @GetMapping("sub/{pid}")
    public ResponseVo<List<CategoryEntity>> querySubCategoriesByPid(@PathVariable("pid") Long pid) {
        return ResponseVo.ok(categoryService.querySubCategoriesByPid(pid));
    }

    @GetMapping("parent/{parentId}")
    @ApiOperation("商品分类查询")
    public ResponseVo<List<CategoryEntity>> queryCategoriesByParentId(@PathVariable("parentId") Long parentId) {
//        return ResponseVo.ok(CacheUtils.getDataListFromCache(redisTemplate, TOP_CACHE_KEY_PREFIX, parentId, CategoryEntity.class, categoryService, "queryCategoriesByParentId", false));
        return ResponseVo.ok(categoryService.queryCategoriesByParentId(parentId));
    }
